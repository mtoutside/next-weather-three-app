...
// 球体用のFBMシェーダー（ライティング付き）
export const sphereFbmFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // FBMベースカラー
    vec3 fbmColor = base * mix(0.7, 1.35, n * glow);
    fbmColor = mix(fbmColor, vec3(dot(fbmColor, vec3(0.299, 0.587, 0.114))), wind * 0.2);

    // シンプルなライティング計算
    vec3 lightDirection = normalize(vec3(5.0, 5.0, 5.0) - vWorldPosition);
    float lambertian = max(dot(vNormal, lightDirection), 0.0);

    // 環境光とディレクショナルライト
    float ambientStrength = 0.6;
    float lightStrength = 0.8;
    float lighting = ambientStrength + lightStrength * lambertian;

    // 最終カラー
    vec3 finalColor = fbmColor * lighting;
